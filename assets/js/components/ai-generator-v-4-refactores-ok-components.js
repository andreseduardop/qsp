...
function safeParseJson(raw) {
  // Comentario: elimina BOM y marcas de bloque ````json```
  const noBom = raw.replace(/^\uFEFF/, "");
  const noFences = noBom.replace(/```(?:json)?/gi, "");

  // Comentario: intenta localizar el primer { o [ y el último } o ]
  const start = Math.min(
    ...[noFences.indexOf("{"), noFences.indexOf("[")].filter((i) => i !== -1),
  );
  const end = Math.max(noFences.lastIndexOf("}"), noFences.lastIndexOf("]"));

  const slice = start >= 0 && end > start ? noFences.slice(start, end + 1) : noFences;
...
class Controller {
  ...
  async _onSubmitDetails(text) {
    ...
    try {
      ...
      console.debug("[ai-generator] llamanddo a Prompt API con prompt: ", promptText1);
      const jsonReceived = await queryPrompt(promptText1);
      parsed = safeParseJson(String(jsonReceived));
    } catch (err) {
    ...
    try {
      ...
      components = this._cloneAndFilterTools(parsed);
      // eslint-disable-next-line no-console
      console.debug("[ai-generator] selected tools:", components);
    } catch (err) {
      // eslint-disable-next-line no-console
    ...
    const promptText2 = `
      ...
      Activity to plan: ${eventType}. Activity details: ${eventDetails}.\n\n
      INSTRUCTIONS:\n
      1. The JSON output MUST be a complete array of objects that EXACTLY follows the structure of outputSchema. The output must start with '[{"name":...' and end with '}]'.\n
      2. The "content" key for each tool must be FILLED with RELEVANT data for the Activity details, matching the data type (object or array) defined in the outputSchema.\n
      3. DO NOT copy the placeholder content (e.g., "task 1" or "Activity 1 description text") from the outputSchema; generate new, specific content.\n
    ...
    try {
      console.debug("[ai-generator] llamanddo a Prompt API con prompt: ", promptText2);
      componentsContent = await queryPrompt(promptText2);
      console.debug("componentsContent: ", componentsContent);
      componentsParsed = safeParseJson(String(componentsContent));
      console.debug("parsed: ", componentsParsed);
    } catch (err) {
