 ...
 * @property {number=} temperature Sampling temperature (if supported).
 * @property {number=} topK Top-K sampling (if supported).
...
class LanguageModelAdapter extends AbstractPromptAdapter {
  ...
  /** @override */
  async availability() {
    // Comentario (ES): consulta disponibilidad tal como indica la documentación.
...
class PromptService {
  ...
  async init_(opts) {
    ...
    this.session_ = await this.adapter_.createSession(opts);
 ...
 * });
 */
export async function queryPrompt(prompt, opts = undefined) {
