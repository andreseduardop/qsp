...
import { setProject } from '../core/storage.js'; // persiste el proyecto por id
...
/**
 * Extrae el posible bloque JSON de una cadena con o sin fence ```json.
 * @param {string} raw
 * @return {string}
 */
...
/**
 * Intenta reparar y parsear JSON desde una cadena posiblemente decorada.
 * @param {string} raw
 * @returns {unknown}
 * @throws {SyntaxError}
 */
  ...
  async _onSubmitDetails(text) {
    ...
    const normalizedComponents = Array.isArray(componentsParsed)
      ? componentsParsed.map((comp, idx) => {
          ...
          // Comentario: añade position (1..n) y state 'mounted'
          return {
            ...rest,
            position: idx + 2,
            state: 'mounted',
    ...
    // Comentario: asigna el id y reemplaza la lista completa de componentes normalizados
    model.id = newId;
    model.createdAt = model.updatedAt = new Date().toISOString(); // asigna fecha/hora actual a las dos claves

    model.components = deepClone(normalizedComponents);
...
export function renderAiGenerator(_containerEl) {
  // Comentario: mantiene firma pública y respeta el layout implícito
